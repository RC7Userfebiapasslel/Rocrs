...
public class FrontendProxyMiddleware
{
    ...
    private async Task<HttpResponseMessage> ProxyRequestAsync(string url, HttpContext ctx)
    {
    ...
    var request = new HttpRequestMessage(HttpMethod.Get, safeUrl);
    if (ctx.Request.Headers.ContainsKey("Cookie"))
        request.Headers.Add("Cookie", ctx.Request.Headers["Cookie"].ToString());
...
#endif
        var result = await ProxyRequestAsync(requestUrl, ctx);
